...
```bash
# Get ChallengeValidation resource
kubectl get challengevalidation -n challenge-<name>

# View detailed status
kubectl get challengevalidation <name> -n challenge-<name> -o yaml

# Look at the status.conditions field for specific failures
```

**2. Check operator logs**
```bash
# View operator logs
kubectl logs -n kubeasy-system -l app=challenge-operator --tail=50

# Follow logs in real-time
kubectl logs -n kubeasy-system -l app=challenge-operator -f
```

**3. Verify resources are as expected**
```bash
...
kubectl logs <pod-name> -n challenge-<name>
```
...
2. **Gather diagnostic info**
   ...
   # Challenge state
   kubectl get all,challengevalidations -n challenge-<name> -o yaml > challenge-state.yaml
   ```
