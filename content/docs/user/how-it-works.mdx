...
| Type | What it checks |
|------|----------------|
| `status` | Resource status fields with operators (replicas, restartCount, phase) |
| `condition` | Kubernetes conditions (Pod Ready, Deployment Available) |
| `log` | Strings in container logs |
| `event` | Forbidden Kubernetes events (OOMKilled, CrashLoopBackOff) |
...
# In challenge.yaml
validations:
  # Check a condition (Ready, Available, etc.)
  - key: pod-ready
    title: "Pod Ready"
  ...
  # Check a status field with operator
  - key: low-restarts
